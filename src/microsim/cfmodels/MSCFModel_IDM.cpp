...
MSCFModel_IDM::insertionFollowSpeed(const MSVehicle* const v, double speed, double gap2pred, double predSpeed, double predMaxDecel, const MSVehicle* const pred) const {
    ...
    double twoSqrtAccelDecel = 2 * sqrt(v->getMaxAccel() * myDecel);
    double s = MAX2(0., speed * myHeadwayTime + speed * (speed - predSpeed) / twoSqrtAccelDecel);
    if (gap2pred >= s) {
...
MSCFModel_IDM::stopSpeed(const MSVehicle* const veh, const double speed, double gap, double decel) const {
    ...
    if (gap > 0 && speed < NUMERICAL_EPS && result < NUMERICAL_EPS) {
        ...
        result = maximumSafeStopSpeed(gap, decel, speed, veh->getMaxAccel(), false, veh->getActionStepLengthSecs());
    }
    //if (result * TS > gap) {
...
MSCFModel_IDM::getSecureGap(const MSVehicle* const veh, const MSVehicle* const /*pred*/, const double speed, const double leaderSpeed, const double /*leaderMaxDecel*/) const {
    ...
    return MAX2(0.0, speed * myHeadwayTime + speed * delta_v / twoSqrtAccelDecel);
}
...
// we assume however constant speed for the leader
    ...
    for (int i = 0; i < myIterations; i++) {
        ...
        double twoSqrtAccelDecel = 2 * sqrt(veh->getMaxAccel() * myDecel);
        double s = MAX2(0., newSpeed * headwayTime + newSpeed * delta_v / twoSqrtAccelDecel);
        if (respectMinGap) {
            s += myType->getMinGap();
        }
        gap = MAX2(NUMERICAL_EPS, gap); // avoid singularity
        const double acc = veh->getMaxAccel() * (1. - pow(newSpeed / desSpeed, myDelta) - (s * s) / (gap * gap));
#ifdef DEBUG_V
        if (gDebugFlag1) {
